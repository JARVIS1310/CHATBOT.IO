...
async function fetchDuckDuckGoAnswer(query) {
  ...
  try {
    ...
    if (abstract) {
      return { text: data.AbstractText, source: data.AbstractSource || 'DuckDuckGo' };
    }
    const related = Array.isArray(data?.RelatedTopics) ? data.RelatedTopics : [];
    for (const topic of related) {
      if (typeof topic?.Text === 'string' && topic.Text.trim()) {
        return { text: topic.Text.trim(), source: 'DuckDuckGo' };
      }
    }
    return null;
  } catch {
...
async function fetchWikipediaAnswer(query) {
  ...
  try {
    ...
    return { text: extract, source: 'Wikipedia', title };
  } catch {
    return null;
  } finally {
    timeout.clear();
...
function buildReply(message, history) {
  ...
  return {
    reply: [intro, '', supportLine, ...lines.map((line) => `- ${line}`), '', `Next step: ${suggestions[0]}`].join('\n'),
    suggestions,
    topic,
    mode: 'free',
  };
...
async function buildGeneralReply(message) {
  ...
  if (duck) {
    return {
      reply: ['Here is what I found:', '', duck.text, '', `Source: ${duck.source}`].join('\n'),
      suggestions: [`Explain this in simpler words`, `Give me 3 key points about this`, `How is this useful in real life?`],
      topic: 'general',
      mode: 'free',
    };
  }
  ...
  if (wiki) {
    return {
      reply: ['Here is a concise answer:', '', wiki.text, '', `Source: ${wiki.source}${wiki.title ? ` (${wiki.title})` : ''}`].join('\n'),
      suggestions: [`Summarize this in 5 bullets`, `Give me a beginner explanation`, `What should I learn next about this?`],
      topic: 'general',
      mode: 'free',
    };
  }
  ...
  return {
    reply: [`I could not fetch a reliable external answer right now, but I can still help.`, '', `Try asking with a bit more detail, for example:`, `- Who is [person]?`, `- Explain [topic] in simple terms.`, `- Compare [A] vs [B].`].join('\n'),
    suggestions: [`Explain this like I am a beginner`, `Give a short and detailed version`, `Ask me clarifying questions first`],
    topic: 'general',
    mode: 'free',
  };
}
